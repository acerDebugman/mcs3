...
impl Bucket {
    ...
    #[maybe_async::async_impl]
    pub async fn mc_put_object_stream<R: AsyncRead + Unpin>(
        &self,
        reader: &mut R,
        s3_path: impl AsRef<str>,
    ) -> Result<u16, S3Error> {
        self._put_object_stream_with_content_type_in_sequence(
            reader,
            s3_path.as_ref(),
            "application/octet-stream",
        )
    ...
    #[maybe_async::async_impl]
    async fn _put_object_stream_with_content_type_in_sequence<R: AsyncRead + Unpin>(
        &self,
        reader: &mut R,
        s3_path: &str,
        content_type: &str,
        ...
        loop {
            ...
            // Start chunk upload
            part_number += 1;
            responses.push(self.make_multipart_request(
                &path,
                chunk,
                part_number,
                upload_id,
